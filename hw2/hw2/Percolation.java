...
public class Percolation {
    ...
    public void open(int row, int col) {
        if (row < 0 || row >= this.row || col < 0 || col >= this.col)  throw new IndexOutOfBoundsException("row or col input argument to open() is out of bounds");
        if (isOpen(row, col) == OPEN) return;
        else {
            siteMap[row][col] = OPEN;
            n++;
            updateSiteConnection(row, col);
            if (percolated(row, col)) {
     ...
     * @param col
     */
    public void updateSiteConnection(int row, int col) {
     ...
     */
    public boolean isConnected(int row, int col, char neighbour) {
        switch (neighbour) {
            case 'U':
                if (row - 1 >= 0) {
